...
Let $\phi(r)=0$, dividing both sides by $4\varepsilon$ and rearranging, we obtain
\[
...
\]
indicating an net attractive force at $r_0=\sigma$.

At the equilibrium separation ($r_{\mathrm{eq}}$), the force from LJ potential is 0. To find $r_{\mathrm{eq}}$, we can solve the equation 
\[
...
\begin{figure}[H]
    ...
    \caption{Total energy as a function of simulation time for NPT simulations with different timesteps. After equilibration, the energy fluctuates around a stable mean value for all tested timesteps, indicating numerical stability. All simulations yielded a qualitatively similar fluctuating pattern regardless of the timestep.}
...
Physically, the timestep is the internal of physical time over which the equations of motion are integrated during the simulation. The unit of timestep $\Delta t$ is defined as:~\cite{frenkel2023understanding}
\[
\Delta t=\sqrt{\frac{m\sigma^2}{\varepsilon}}.
\]
At each timestep, the forces of the atoms are calculated, and positions and/or velocities are updated discretely. $\Delta t$ represents the time between two successive updates of the state of the system. 

Hence if the timestep is too large, the force calculations would become inaccurate, and the simulation is no longer valid physically. This effect is especially significant for LJ system, as it has a very strong repulsive term at short distances (e.g., at $r=0.9\ \sigma$, the LJ potential is already $5\ \varepsilon$). Therefore, the timestep must be small enough to \textit{resolve} the motions of the atoms.
...
decreases. This is due to the fact that at higher temperatures the kinetic energy dominates over the interparticle potential energy, so the system behaves more like an ideal gas.
...
\begin{figure}[H]
    ...
    \caption{Radial distribution functions $g(r)$ for the solid (FCC), liquid, and gas phases of the LJ system. Algorithmatic cutoffs have been observed for all three phases simulated.}
...
The RDF of the solid phase displays multiple sharp peaks up to approximately $r^*=5.5$, reflecting the long-range periodic structure of the FCC lattice. 

The liquid phase exhibits a strong first peak and damped oscillations corresponding to short-range order. 

It is found that both the solid and liquid phases are cut off at a certain radius, illustrating the finite size of the simulation box and the maximum distance over which the RDF is computed.
...
\begin{table}[H]
\centering
\caption{Simulated radial positions of the first three RDF shells and the corresponding coordination numbers for the FCC solid.}
...
\caption{MSD as a function of reduced time $t^*$ for the solid, liquid, and gas phases at different timesteps $\Delta t$.}
\end{subfigure}

\caption{Dynamical properties of the LJ system obtained from MD simulations at different timesteps.}
\end{figure}
...
The selection of different cutoff distances reflects a trade-off between computational cost and accuracy. Using a shorter cutoff reduces computational cost because fewer pairwise interactions need to be calculated. However, truncating the potential too aggressively may neglect part of the long-range dispersion interactions arising from the $r^{-6}$ attractive term, which can introduce errors in thermodynamic and kinetic properties. Increasing the cutoff distance improves the accuracy of the potential by including more of these long-range interactions, but at the expense of higher computational cost.


\subsubsection*{In this lab you have investigated the properties of solids, liquids and gases. Would you observe a liquid phase (and by extension a critical point) if the LJ interaction strength, $\varepsilon$, is very weak? What interaction strength is required to generate a liquid phase? [4]}

Since a distinct liquid state is only formed below the critical point, intermolecular interactions must be sufficiently strong to keep the molecules bound together, otherwise they would have been separated and exist as a gas. These intermolecular interactions are characterised by LJ interaction strength $\varepsilon$, and for liquid, the interaction strength must be at least comparable to thermal energy of the particles. This relation can be qualitatively expressed by
\[
\frac{k_BT}{\varepsilon}\sim1,
\]
where the left-hand side correpsonds to the definition of the temperature in reduced unit, $T^*$, in the simulation.
\[
T^*=\frac{k_B T}{\varepsilon}.
\]
It is now well established that for LJ systems, the critical temperature in reduced units is approximately $T^*_c\approx1.31$~\cite{perez2006critical}. Therefore, to generate a liquid phase, the reduced temperature must satisfy $T^*<T^*_c$; otherwise liquid and gas phases would become indistinguishable.

When the interaction strength is very weak, the reduced temperature effectively becomes very high since $T^*\propto\varepsilon^{-1}$. The system will remain above $T^*_c$, i.e., above the critical point, where liquid phases can not exist. The system will then exhibit weakly-interacting gas-like behaviours.

To generate a liquid phase, this inequality must be satisfied:
\[
\varepsilon\ge\frac{k_B}{T^*_c}T,
\]
i.e., the interaction strength $\varepsilon$ must be sufficiently large compared to the thermal energy of the particles.
...
\subsubsection*{Algorithms such as SHAKE and RATTLE are widely used to model molecular systems as they allow MD simulations to be performed while fixing bond lengths. Why is this desirable? [4]}

SHAKE algorithm achieves more efficient simulation of molecular systems by applying position constraints to freeze some of the very fast motions e.g., bond stretching and bond vibrations, as they i) require very small timesteps to be adopted, and ii) often do not alter the macroscopic properties significantly~\cite{ryckaert1977numerical}. Effectively, the bond lengths can be kept constant throughout the simulation. RATTLE has the same basic philosophy as SHAKE, though velocity computations are involved and extra velocity constraints are applied, thus sometimes they are considered more numerically robust than SHAKE~\cite{andersen1983rattle}.

The main advantage of adopting algorithms like SHAKE and RATTLE is that they allow a much higher timestep to be used, thus reducing the computational cost significantly while retaining similar accuracy. Bond vibrations occur very fast (often $\sim$fs timescale), especially for the lighter atoms like hydrogen, thus imposing very strict restrictions on the timestep. By constraining the atom positions, i.e., by fixing the bond lengths, a much higher timestep can be used without sacraficing on the simulation stability and robustness. Therefore, simulations for a longer timescale and/or a larger particle system can be achieved.
